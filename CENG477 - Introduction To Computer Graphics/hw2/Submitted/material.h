...
namespace fst
{
    ...
    public:
        ...
        math::Vector3f computeBrdf(const math::Vector3f& wi, const math::Vector3f& wo, const math::Vector3f& normal, const math::Vector3f& renk, const std::string& decal) const;

        const math::Vector3f& get_ambient() const { return m_ambient; }
        const math::Vector3f& get_mirror() const { return m_mirror; }
        const math::Vector3f& get_diffuse() const {return m_diffuse;}
        const float& getPhong() const {return m_phong_exponent;}
        const math::Vector3f& get_specular() const {return m_specular;}
    private:
